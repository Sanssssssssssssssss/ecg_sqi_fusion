...
case "${SLURM_ARRAY_TASK_ID}" in
  0)
    RUN_TAG="r6_robust_snr002"
    EXTRA_ARGS=("${BASE_ARGS[@]}" --lambda_snr 0.02)
    ;;
  1)
    RUN_TAG="r6_robust_snr0075"
    EXTRA_ARGS=("${BASE_ARGS[@]}" --lambda_snr 0.075)
    ;;
  2)
    RUN_TAG="r6_robust_med103"
    EXTRA_ARGS=("${BASE_ARGS[@]}" --lambda_snr 0.05 --class_weight_medium 1.03)
    ;;
  3)
    RUN_TAG="r6_robust_med105"
    EXTRA_ARGS=("${BASE_ARGS[@]}" --lambda_snr 0.05 --class_weight_medium 1.05)
    ;;
  4)
    RUN_TAG="r6_robust_ls005"
    EXTRA_ARGS=("${BASE_ARGS[@]}" --lambda_snr 0.05 --label_smoothing 0.005)
    ;;
  5)
    RUN_TAG="r6_robust_med103_snr002"
    EXTRA_ARGS=("${BASE_ARGS[@]}" --lambda_snr 0.02 --class_weight_medium 1.03)
    ;;
  6)
    RUN_TAG="r6_robust_gm103"
    EXTRA_ARGS=("${BASE_ARGS[@]}" --lambda_snr 0.05 --class_weight_good 1.03 --class_weight_medium 1.03)
    ;;
  7)
    RUN_TAG="r6_raw_med103"
