...
BASE_ARGS=(--epochs 24 --lr 3e-5 --lr_eta_min 2e-6 --weight_decay 0.03 --dropout 0.10 --snr_head --e_level 0)

case "${SLURM_ARRAY_TASK_ID}" in
  0)
    RUN_TAG="r7_tiny_denoise_seed1"
    SEED_VALUE=1
    EXTRA_ARGS=("${BASE_ARGS[@]}" --lambda_snr 0.05 --e_cls 16 --e_denoise 8 --lambda_den 10 --bad_den_w_max 0.10 --bad_den_w_warmup_epochs 8)
    ;;
  1)
    RUN_TAG="r7_tiny_denoise_seed2"
    SEED_VALUE=2
    EXTRA_ARGS=("${BASE_ARGS[@]}" --lambda_snr 0.05 --e_cls 16 --e_denoise 8 --lambda_den 10 --bad_den_w_max 0.10 --bad_den_w_warmup_epochs 8)
    ;;
  2)
    RUN_TAG="r7_tiny_denoise_seed3"
    SEED_VALUE=3
    EXTRA_ARGS=("${BASE_ARGS[@]}" --lambda_snr 0.05 --e_cls 16 --e_denoise 8 --lambda_den 10 --bad_den_w_max 0.10 --bad_den_w_warmup_epochs 8)
    ;;
  3)
    RUN_TAG="r7_tiny_den5_bad005"
    SEED_VALUE=0
    EXTRA_ARGS=("${BASE_ARGS[@]}" --lambda_snr 0.05 --e_cls 16 --e_denoise 8 --lambda_den 5 --bad_den_w_max 0.05 --bad_den_w_warmup_epochs 8)
    ;;
  4)
    RUN_TAG="r7_late_den8_bad005"
    SEED_VALUE=0
    EXTRA_ARGS=("${BASE_ARGS[@]}" --lambda_snr 0.05 --e_cls 18 --e_denoise 6 --lambda_den 8 --bad_den_w_max 0.05 --bad_den_w_warmup_epochs 6)
    ;;
  5)
    RUN_TAG="r7_tiny_snr002"
    SEED_VALUE=0
    EXTRA_ARGS=("${BASE_ARGS[@]}" --lambda_snr 0.02 --e_cls 16 --e_denoise 8 --lambda_den 10 --bad_den_w_max 0.10 --bad_den_w_warmup_epochs 8)
    ;;
  *)
