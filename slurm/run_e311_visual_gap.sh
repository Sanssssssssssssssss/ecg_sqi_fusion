...
if not torch.cuda.is_available():
    raise SystemExit("CUDA is required for E3.11 transformer training; refusing to run on CPU.")
PY

ARTIFACT_DIR="${ARTIFACT_DIR:-outputs/transformer_e311_visual_gap}"
SOURCE_ARTIFACT_DIR="${SOURCE_ARTIFACT_DIR:-outputs/transformer}"
EXPERIMENT_NAME="${EXPERIMENT_NAME:-e311_visual_gap_cls_raw}"
SQI_OUT_DIR="${SQI_OUT_DIR:-outputs/transformer_e311_visual_gap_sqi_ml_three_class}"
...
python -u -m src.transformer_pipeline.noise.synthesize_morph_damage_triplet \
  ...
  --label_version e311_smooth_morph_visual_gap \
...
lines = [
    "# E3.11 Visual-Gap Smooth-Morph Audit",
    "",
    ...
    "- target SNR ranges: good `13-15 dB`, medium `7-9 dB`, bad `3-6 dB`",
    "- label axis: E3.9a `smooth_morph_score`",
    "- good is stricter and closer to clean",
    "- bad is visually farther with guarded QRS/correlation triggers",
...
lines.extend([
    ...
    "## Figures",
    "",
    "- `figs_label_samples/e311_counterfactual_triplets.png`",
    "- `figs_label_samples/e311_class_noise_examples.png`",
])
(artifact_dir / "e311_audit_report.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
print(f"Wrote {artifact_dir / 'e311_audit_report.md'}")
PY
