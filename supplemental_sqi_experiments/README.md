...
This directory is intentionally outside `src/sqi_pipeline`.  It contains the
extra protocol audits requested after the paper-aligned SQI reproduction:
...
- `reports/sqi_supplemental/existing_seed0/**/fig_supp_*.png`

Noise-time isolated rerun entrypoints:

```powershell
.\.venv\Scripts\python.exe -m supplemental_sqi_experiments.run build-isolated --seed 0
.\.venv\Scripts\python.exe -m supplemental_sqi_experiments.run run-isolated --seed 0
```
