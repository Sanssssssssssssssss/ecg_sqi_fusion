...
from supplemental_sqi_experiments.common import (
    LEADS_12,
    SQIS,
    load_split_frame,
    project_root,
...
from supplemental_sqi_experiments.plotting import PALETTE, apply_style, save_figure
...
def _plot_single_lead(ax: plt.Axes, sig: np.ndarray, leads: list[str], fs: float, *, lead: str, color: str, label: str, show_x: bool, show_ylabel: bool) -> None:
...
def plot_original_vs_synthetic_bad_gallery(
    ...
    *,
    ...
    for col_idx, (col_name, header, color) in enumerate(columns):
        ...
        axes[0, col_idx].set_title(header, loc="left", fontsize=8.5, fontweight="bold", color=PALETTE["ink"], pad=4)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--shared-images-dir", default=str(root / "reports" / "sqi_paper_aligned" / "images"))
