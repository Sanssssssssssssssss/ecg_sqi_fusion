...
class RunConfig:
    ...
    # model selection
    c_grid: tuple[float, ...] = (1e-4, 1e-3, 1e-2, 1e-1, 1.0, 10.0, 100.0)
...
def outputs_exist(paths: Paths, seed: int) -> bool:
    m = paths.models_dir / f"logreg_best_seed{seed}.pkl"
    met = paths.out_dir / f"logreg_test_metrics_seed{seed}.json"
...
def run(params: dict[str, Any]) -> dict[str, Any]:
    ...
    params (optional):
      ...
      - select_metric: "val_acc" | "val_auc"
      - c_grid: list[float] or tuple[float,...]
    ...
    if (not cfg.force) and outputs_exist(paths, cfg.seed):
        outs = [
            str(paths.models_dir / f"logreg_best_seed{cfg.seed}.pkl"),
            str(paths.out_dir / f"logreg_test_metrics_seed{cfg.seed}.json"),
        ]
        logger.info("logreg: outputs exist -> skip (force=True to rerun)")
        return {"step": "logreg", "skipped": True, "outputs": outs}
    ...
    # ---- C search on val ----
    rows = []
    best = {"C": None, "val_acc": -1.0, "val_auc": -1.0}
    t_search0 = time.time()
    ...
    for C in cfg.c_grid:
        model = LogisticRegression(
            C=float(C),
            penalty="l2",
            solver="liblinear",   # stable for small/medium
            max_iter=2000,
        ...
        metric = row["val_acc"] if cfg.select_metric == "val_acc" else row["val_auc"]
        best_metric = best["val_acc"] if cfg.select_metric == "val_acc" else best["val_auc"]

        # tie-break: metric desc, then auc desc, then smaller C
        if (metric > best_metric) or (metric == best_metric and row["val_auc"] > best["val_auc"]) or (
            metric == best_metric and row["val_auc"] == best["val_auc"] and (best["C"] is None or float(C) < float(best["C"]))
        ):
            best = {"C": float(C), "val_acc": row["val_acc"], "val_auc": row["val_auc"]}

        logger.info("C=%g | val_acc=%.4f val_auc=%.4f", float(C), row["val_acc"], row["val_auc"])

    df_search = pd.DataFrame(rows).sort_values("C").reset_index(drop=True)
    out_search = paths.models_dir / f"logreg_C_search_seed{cfg.seed}.csv"
    df_search.to_csv(out_search, index=False)
    logger.info("[saved] %s", out_search)

    logger.info("Best C=%g by %s (val_acc=%.4f val_auc=%.4f) | search_time=%.2fs",
                best["C"], cfg.select_metric, best["val_acc"], best["val_auc"], time.time() - t_search0)
    ...
    out_model = paths.models_dir / f"logreg_best_seed{cfg.seed}.pkl"
    ...
    out_probs = paths.probs_dir / f"logreg_test_probs_seed{cfg.seed}.npz"
    ...
    out_roc = paths.roc_dir / f"logreg_roc_seed{cfg.seed}.png"
    plot_roc(yte, p_test, out_roc, title="LogReg ROC (test)")
    outs.append(str(out_roc))
    logger.info("[saved] %s", out_roc)

    out_roc_maxacc = paths.roc_dir / f"logreg_roc_maxacc_seed{cfg.seed}.png"
    best_thr_full = plot_roc_with_maxacc(yte, p_test, out_roc_maxacc, n_grid=cfg.maxacc_grid)
    outs.append(str(out_roc_maxacc))
    logger.info("[saved] %s", out_roc_maxacc)

    out_maxacc = paths.maxacc_dir / f"logreg_maxacc_seed{cfg.seed}.csv"
    pd.DataFrame([best_thr_full]).to_csv(out_maxacc, index=False)
    outs.append(str(out_maxacc))
    logger.info("[saved] %s", out_maxacc)

    out_metrics = paths.out_dir / f"logreg_test_metrics_seed{cfg.seed}.json"
    ...
    logger.info("=== LOGREG FINAL TEST (once) ===")
    logger.info("C=%g | acc=%.4f se=%.4f sp=%.4f auc=%.4f",
                float(best["C"]),
                test_metrics_fixed["acc"], test_metrics_fixed["se"], test_metrics_fixed["sp"], test_metrics_fixed["auc"])

    return {"step": "logreg", "skipped": False, "outputs": outs}
