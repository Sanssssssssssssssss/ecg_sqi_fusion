...
def render(rows: list[dict[str, Any]]) -> str:
    ...
    lines.append("| `29777633` | `sqi_mil_mlp_lrgroup` | `0-7%2` | first two completed below `0.936`; remaining low-value variants canceled |")
    lines.append("| `29780940` | `sqi_residual_refined` | `0-7%2` | conservative zero-init SQI residual correction on top of loaded D1 classifier logits |")
    lines.append("| `29787855` | `sqi_residual_freeze` | `0-5%2` | freeze shared warm-started model and train only SQI residual/head parameters |")
    ...
    lines.append("- `sqi_mil_mlp_lrgroup` tests whether the direct SQI+MLP head failed because the randomly initialized head and warm-started backbone were optimized at the same learning rate.")
    lines.append("- `sqi_residual_refined` preserves the loaded D1 `cls_fc` and adds only a zero-init SQI residual delta, so useful SQI information can correct logits without replacing the main classifier.")
    lines.append("- `sqi_residual_freeze` checks whether SQI residual features help only when they are prevented from moving the warm-started encoder/decoder and classifier too far.")
