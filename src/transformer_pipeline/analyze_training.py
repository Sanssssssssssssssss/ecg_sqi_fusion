...
def summarize_dir(root: Path, model_dir: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "cls_pooling": hp.get("cls_pooling"),
        "local_pool_topk": hp.get("local_pool_topk"),
        "positional_embedding": hp.get("positional_embedding"),
...
def load_ckpt_hyperparams(model_dir: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "cls_pooling": hp.get("CLS_POOLING"),
        "local_pool_topk": hp.get("LOCAL_POOL_TOPK"),
        "positional_embedding": hp.get("USE_POSITIONAL_EMBEDDING"),
        "lambda_den": hp.get("LAMBDA_DEN"),
        "bad_den_w_max": hp.get("BAD_DEN_W_MAX"),
        "label_smoothing": hp.get("LABEL_SMOOTHING"),
        "class_weight_medium": hp.get("CLASS_WEIGHT_MEDIUM"),
...
def print_table(rows: list[dict[str, Any]]) -> None:
    ...
    for row in rows:
        ...
        table.append([
            ...
            str(row.get("cls_pooling") or ""),
            "1" if row.get("positional_embedding") else "0",
